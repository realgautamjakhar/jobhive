...
const JobCard = ({ job }: { job: Job }) => {
  return (
    <motion.li
      ...
      className=" grid items-center gap-2 rounded-2xl bg-white p-4 shadow-2xl shadow-accent-100/50 hover:shadow-accent-100 md:grid-cols-[1fr_auto]"
    >
      <Link
        href={`/job/${job.id}`}
        className="grid  grid-cols-[auto_1fr] grid-rows-[3,auto] items-center gap-x-4  "
      >
        <Image
          width={80}
          height={80}
        ...
        <ul className=" col-span-2 my-2 flex flex-wrap items-center justify-center gap-4 md:col-span-1 md:my-[revert] md:justify-start">
          <TextItem
            prefix="Salary"
